...
const FOUNDATION_MODULES: Record<string, ModuleDef> = {
  ...
  ai: {
    title: 'PolyON AI',
    desc: 'AI Gateway · 모델 · 에이전트',
    section: 'SERVICES',
    defaultPath: '/ai',
    icon: Bot,
    serviceId: 'ai',
    items: [
      { label: '개요', path: '/ai', icon: Bot },
      { type: 'divider' },
      { label: '모델', path: '/ai/models', icon: DataBase },
      { label: 'API 키', path: '/ai/keys', icon: Key },
      { label: '사용량', path: '/ai/usage', icon: ChartLine },
      { type: 'divider' },
      { label: '에이전트', path: '/ai/agents', icon: Bot },
      { label: '메모리', path: '/ai/memory', icon: Document },
      { label: '파이프라인', path: '/ai/pipeline', icon: Activity },
      { type: 'divider' },
      { label: '설정', path: '/ai/settings', icon: Settings },
    ],
  },
  security: {
    title: 'Security',
